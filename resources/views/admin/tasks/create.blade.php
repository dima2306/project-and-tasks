...
@section('content')
    <div class="max-w-lg mx-auto mt-8 bg-white dark:bg-gray-900 p-6 rounded shadow">
        ...
        <h2 class="text-xl font-bold mb-6 text-gray-900 dark:text-gray-100">ახალი დავალების შექმნა</h2>
            ...
            <div class="mb-4">
                ...
                >
                    ...
                    @foreach($projects as $project)
                        <option value="{{ old('project_id') ?? $project->id }}">{{ $project->name }}</option>
                    @endforeach
                </select>
                @error('project_id')
            ...
            <div class="mb-4">
                ...
                >
                    ...
                    @foreach($statuses as $value => $status)
                        <option value="{{ old('status') ?? $value }}">{{ $status }}</option>
                    @endforeach
                </select>
                @error('status')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
