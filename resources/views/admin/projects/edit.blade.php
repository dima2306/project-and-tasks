...
@section('content')
    <div class="max-w-lg mx-auto mt-8 bg-white dark:bg-gray-900 p-6 rounded shadow">
        ...
        <h2 class="text-xl font-bold mb-6 text-gray-900 dark:text-gray-100">
            "{{ $project->name }}" პროექტის რედაქტირება
        </h2>
        <form action="{{ route('admin.projects.update', $project->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-500 dark:bg-gray-800 dark:text-gray-100">{{ old('description') ?? $project->description }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
